  ...
  async function fetchActivities() {
    try {
      ...
      const participantsHtml = details.participants.length > 0
        ? `<p><strong>Participants:</strong></p><ul class="participants-list">${details.participants.map(email => {
            const encodedEmail = encodeURIComponent(email);
            const encodedActivity = encodeURIComponent(name);
            return `<li><span>${email}</span><button class="participant-remove" data-activity="${encodedActivity}" data-email="${encodedEmail}" title="Unregister ${email}">×</button></li>`;
          }).join("")}</ul>`
